...
import jwt, json, redis, rq, secrets
...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    def launch_task(self, name, description, *args, **kwargs):
        rq_job = current_app.task_queue.enqueue(f'app.task.{name}', self.id, *args, **kwargs)
...
class Task(db.Model):
    ...
    def get_progress(self):
        job = self.get_rq_job()
        return job.meta.get('progess', 0) if job is not None else 100